PathControls = function (object, domElement) {
	this.object = object;
	this.domElement = (domElement !== undefined) ? domElement : document;

	this.id = "PathControls" + PathControlsIdCounter ++;
	...
	this.debugPath = new Object3D();
	this.debugDummy = new Object3D();

	this.animationParent = new Object3D();
	...
	this.update = function (delta) {
		...
		var tmpPhi = Math.mapLinear(this.phi, srcRange[0], srcRange[1], dstRange[0], dstRange[1]);
		...
		var tmpTheta = Math.mapLinear(this.theta, srcRange[0], srcRange[1], dstRange[0], dstRange[1]);
	...
	function initAnimationPath(parent, spline, name, duration) {
		...
		return new Animation(parent, name, AnimationHandler.CATMULLROM_FORWARD, false);
	};
	...
	function createPath(parent, spline) {
		var lineGeo = createSplineGeometry(spline, 10),
			...
			lineMat = new LineBasicMaterial({ color: 0xff0000, linewidth: 3 }),
			lineObj = new Line(lineGeo, lineMat),
			particleObj = new ParticleSystem(particleGeo, new ParticleBasicMaterial({ color: 0xffaa00, size: 3 }));
		...
		var waypoint,
			geo = new SphereGeometry(1, 16, 8),
			mat = new MeshBasicMaterial({ color: 0x00ff00 });

		for (var i = 0; i < spline.points.length; i ++) {
			waypoint = new Mesh(geo, mat);
	...
	this.init = function () {
		...
		if (this.createDebugDummy) {
			var dummyParentMaterial = new MeshLambertMaterial({ color: 0x0077ff }),
			dummyChildMaterial  = new MeshLambertMaterial({ color: 0x00ff00 }),
			dummyParentGeo = new CubeGeometry(10, 10, 20),
			dummyChildGeo  = new CubeGeometry(2, 2, 10);

			this.animationParent = new Mesh(dummyParentGeo, dummyParentMaterial);

			var dummyChild = new Mesh(dummyChildGeo, dummyChildMaterial);
