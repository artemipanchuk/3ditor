...
Loader.prototype = {
	...
	initMaterials: function (scope, materials, texturePath) {
		...
		for (var i = 0; i < materials.length; ++ i) {
			scope.materials[i] = Loader.prototype.createMaterial(materials[i], texturePath);
		}
	},
	...
	createMaterial: function (m, texturePath) {
		...
		function create_texture(where, name, sourceFile, repeat, offset, wrap) {
			...
			if(repeat) {
				...
				if (repeat[0] != 1) where[name].wrapS = RepeatWrapping;
				if (repeat[1] != 1) where[name].wrapT = RepeatWrapping;
			}
		...
		if (m.blending !== undefined &&  window[m.blending] !== undefined) {
			mpars.blending =  window[m.blending];
		}
		...
		if (m.vertexColors !== undefined) {
			if (m.vertexColors == "face") {
				mpars.vertexColors = FaceColors;
			} else if (m.vertexColors) {
				mpars.vertexColors = VertexColors;
			}
		}
		...
		if (m.mapNormal) {
			...
			var material = new  window[mtype](mpars);
		}
