CanvasRenderer = function (parameters) {
	...
	this.render = function (scene, camera) {
		...
		for (e = 0, el = _elements.length; e < el; e++) {
			...
			material = material instanceof MeshFaceMaterial ? element.faceMaterial : material;
		...
		function renderFace3(v1, v2, v3, uv1, uv2, uv3, element, material, scene) {
			...
			if (material instanceof MeshBasicMaterial) {
				if (material.map) {
					if (material.map.mapping instanceof UVMapping) {
						_uvs = element.uvs[0];
					...
					if (material.envMap.mapping instanceof SphericalReflectionMapping) {
				...
				if (_enableLighting) {
					if (!material.wireframe && material.shading == SmoothShading && element.vertexNormalsWorld.length == 3) {
		...
		function renderFace4(v1, v2, v3, v4, v5, v6, element, material, scene) {
			...
			if (material instanceof MeshBasicMaterial) {
				...
				if (_enableLighting) {
					if (!material.wireframe && material.shading == SmoothShading && element.vertexNormalsWorld.length == 4) {
